...
public class SpectralBlades extends ArmorAbility {
	...
	protected void activate(ClassArmor armor, Hero hero, Integer target) {
		...
		Char enemy = findChar(b, hero, 4*hero.pointsInTalent(Talent.PROJECTING_BLADES), targets);
		...
		if (hero.hasTalent(Talent.FAN_OF_BLADES)){
			ConeAOE cone = new ConeAOE(b, 60*hero.pointsInTalent(Talent.FAN_OF_BLADES));
			for (Ballistica ray : cone.rays){
				Char toAdd = findChar(ray, hero, 4*hero.pointsInTalent(Talent.PROJECTING_BLADES), targets);
