...
public class ShadowClone extends ArmorAbility {
	...
	public static class ShadowAlly extends DirectableAlly {
		...
		public int damageRoll() {
			...
			heroDamage = Math.round(0.16f * Dungeon.hero.pointsInTalent(Talent.SHADOW_BLADE) * heroDamage);
			if (heroDamage > 0){
				damage += heroDamage;
			}
			return damage;
		...
		public int drRoll() {
			...
			heroRoll = Math.round(0.24f * Dungeon.hero.pointsInTalent(Talent.CLONED_ARMOR) * heroRoll);
			if (heroRoll > 0){
				dr += heroRoll;
			}
			return dr;
