...
public class Tengu extends Mob {

	{
		...
		HP = HT = Dungeon.isChallenged(Challenges.STRONGER_BOSSES) ? 1500 : 750;
		EXP = 20;
		defenseSkill = 15;
	...
	public boolean canUseAbility(){
		...
		if (abilitiesUsed >= targetAbilityUses()){
			...
			if (targetAbilityUses() - abilitiesUsed >= 4 && !Dungeon.isChallenged(Challenges.STRONGER_BOSSES)){
				...
				if (abilityCooldown == -1) abilityCooldown = Random.IntRange(1, 2);
			}
	...
	private int targetAbilityUses(){
		...
		int targetAbilityUses = 6 + 4*arenaJumps;
